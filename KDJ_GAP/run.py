...
sys.path.append('../telegram_utils')
sys.path.append('../')
import json
from log_utils import setup_logger
from datetime import datetime, time, timedelta
from telegram_utils import message as send_message
global strategy_name

from KDJ_GAP import KDJ_GAP
...
if __name__ == "__main__":
    ...
    send_message(f'{strategy_name} strategy started')
            ...
            if dummy_instance.profile[message.topic]['state'] == 'Monitoring Signal':
                ...
                if datetime.now().date() > logic_time.date():
                    ...
                    if dummy_instance.profile[message.topic]['parameters']['trade_time'] == 'morning':
                        if (time(9,30) <= datetime.now(dummy_instance.timezone).time()) and (datetime.now(dummy_instance.timezone).time()<= time(9,35)):
                            send_message(f"{strategy_name} checking trade logic in morning")
                    ...
                    if dummy_instance.profile[message.topic]['parameters']['trade_time'] == 'afternoon':
                        if (time(15,50) <= datetime.now(dummy_instance.timezone).time()) and (datetime.now(dummy_instance.timezone).time()<= time(16,00)):
                            send_message(f"{strategy_name} checking trade logic in afternoon")
                            trade_logic = dummy_instance.check_trade_logic(dummy_instance.profile[message.topic])
                            update_last_logic_time(strategy_obj=dummy_instance.profile[message.topic])
                            dummy_instance.profile[message.topic]['last_logic_time'] = datetime.now().strftime(format_str)
                    if trade_logic:
                        ...
                        send_message(f"{strategy_name} {message.topic} signal detected")
                ...
                if bid_ask_isvalid(bid_ask_dict, message.topic):
                    ...
                    send_message(f"{strategy_name} {message.topic} Opening Order")
                else:
                    if message_key == "bid":
                        #print('updating bid')
                ...
                if status:
                    ...
                    if order_status:
                        ...
                        send_message(f"{strategy_name} {message.topic} Open Position")
                else:
                ...
                if bid_ask_isvalid(bid_ask_dict, message.topic):
                    ...
                    if (time(15,5) > datetime.now(dummy_instance.timezone).time()) and (time(16,00) < datetime.now(dummy_instance.timezone).time()):
                    ...
                    if sell_logic:
                        ...
                        if status:
                            ...
                            send_message(f"{strategy_name} {message.topic} Selling Order")
                        else:
                            print("API Error")
                else:
                    if message_key == "bid":
                ...
                if status:
                    ...
                    if order_status:
                        ...
                        send_message(f"{strategy_name} {message.topic} Order Sold")
                else:
                    print(message.topic,'has problem')
                    pass